myApp.controller('ExpenseCntrl', ['$scope', '$http', '$stateParams', '$timeout', '$rootScope', '$state', 'myService', 'config',
    function ($scope, $http, $stateParams, $timeout, $rootScope, $state, myService, config) {
        ...
        $scope.accountTable = [];
        $scope.expenseDate = moment().format('MM/DD/YYYY');

        $('#expenseDate').datepicker("setDate", new Date());

        $scope.expenseDueDate = moment().format('MM/DD/YYYY');

        $('#expenseDueDate').datepicker("setDate", new Date());

        $('#expenseDueDate').datepicker().on('changeDate', function (ev) {
            $('.datepicker').hide();
        });

        $('#expenseDate').datepicker().on('changeDate', function (ev) {
            $('.datepicker').hide();
        });
        ...
        $scope.$watch('tdsRate.selected', function () {
                ...
                $scope.netTds = Number($scope.totalAmountTds) - Number($scope.tdsamount) + Number($scope.totaltax);
   ...
   // $http.get(config.api + "transactions" + "/count" + "?where[ordertype]=" + "EXPENSE").then(function (response) {
    ...
    $scope.saveExpenceNew = function () {
        ...
        var data = {
            ...
            expenseDueDate:$scope.expenseDueDate,
