...
$(function () {
    ...
    //$("#OrderMenu li").addClass('active').siblings().removeClass('active');
...
function showoverlay(Id, effect) {
    if (effect != undefined)
        run_waitMe(effect, Id);
    else
        run_waitMe(current_effect, Id);
}

function showoverlayId(effect) {
    if (effect != undefined)
        run_waitMe(effect);
    else
...
function hideoverlay(Id) {
    if (Id != undefined) {
        $.each(suboverlay, function (id, ele) {
            if (ele == Id) {
                $(ele).waitMe('hide');
                suboverlay.splice(id, 1)
                return;
                //suboverlay.remove(ele);
            }
        });
